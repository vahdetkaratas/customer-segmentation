...
def create_sample_dataset(n_customers=1000, n_transactions=5000):
    ...
    print(f"Generated {len(df)} transactions for {df['CustomerID'].nunique()} unique customers")
    return df
...
def clean_dataset(df):
    ...
    df_clean = df_clean[
        (df_clean['Quantity'] <= qty_upper) & 
        (df_clean['UnitPrice'] <= price_upper)
    ]
    
    print(f"Final cleaned dataset: {len(df_clean)} transactions for {df_clean['CustomerID'].nunique()} customers")
    return df_clean

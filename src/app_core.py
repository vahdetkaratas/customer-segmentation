...
def load_or_sample_data(path: Optional[str] = None) -> pd.DataFrame:
    """
...
def run_kmeans(rfm_scaled: pd.DataFrame, k: int, random_state: int = 42) -> Tuple[np.ndarray, KMeans]:
    """
...
def run_dbscan(rfm_scaled: pd.DataFrame, eps: float, min_samples: int) -> Tuple[np.ndarray, DBSCAN]:
    """
...
def run_hierarchical(rfm_scaled: pd.DataFrame, n_clusters: int) -> Tuple[np.ndarray, AgglomerativeClustering]:
    """
...
def save_app_artifacts(rfm_df: pd.DataFrame, rfm_scaled: pd.DataFrame, 
                      labels: np.ndarray, customer_ids: pd.Series) -> None:
    """

    ...
    if source == "kmeans":
        ...
        if not Path(file_path).exists():
            raise FileNotFoundError(
                f"KMeans clustering file not found: {file_path}. Please run Step 3 (KMeans clustering) first."
            )
        ...
        if not Path(file_path).exists():
            raise FileNotFoundError(
                f"DBSCAN clustering file not found: {file_path}. Please run Step 4 (Alternative clustering) first."
            )
        df = pd.read_csv(file_path)
        ...
        if not Path(file_path).exists():
            raise FileNotFoundError(
                f"Hierarchical clustering file not found: {file_path}. Please run Step 4 (Alternative clustering) first."
            )
        df = pd.read_csv(file_path)
        ...
        raise ValueError(
            f'Invalid clustering source: {source}. Use "kmeans", "dbscan", or "hierarchical"'
        )
...
def calculate_segment_profiles(df):
    ...
    for segment in sorted(df["segment"].unique()):
        ...
        profile = {
            ...
            "high_recency_share": (
                segment_data["Recency"] > overall_recency_75th
            ).mean(),
        }
...
def create_visualizations(segment_profiles, data, cluster_source):
    ...
    bars = plt.bar(
        segment_profiles["segment"],
        segment_profiles["revenue_share"],
        color=plt.cm.Set3(np.linspace(0, 1, len(segment_profiles))),
    )
    plt.xlabel("Segment")
    ...
    plt.tight_layout()
    plt.savefig(
        "reports/figures/segments_revenue_share.png", dpi=300, bbox_inches="tight"
    )
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig(
        "reports/figures/segments_rfm_heatmap.png", dpi=300, bbox_inches="tight"
    )
    plt.close()
    ...
    plt.suptitle(
        f"RFM Distributions by Segment ({cluster_source.title()} Clustering)",
        fontsize=16,
    )
    plt.tight_layout()
    plt.savefig(
        "reports/figures/segments_rfm_boxplots.png", dpi=300, bbox_inches="tight"
    )
    plt.close()
...
def generate_markdown_report(cluster_source, profiles_df, narratives_df):
    ...
    report_lines.append("")
    report_lines.append(
        f"This report presents the results of customer segmentation analysis using {cluster_source.title()} clustering."
    )
    report_lines.append(
        f"The analysis identified {len(profiles_df)} distinct customer segments based on RFM (Recency, Frequency, Monetary) metrics."
    )
    report_lines.append("")
    ...
    report_lines.append("")
    report_lines.append(
        f'- **Highest Revenue Segment:** Segment {max_revenue_segment["segment"]} contributes {max_revenue_segment["revenue_share"]:.1%} of total revenue'
    )
    report_lines.append(
        f'- **Largest Segment:** Segment {max_customers_segment["segment"]} has {max_customers_segment["n_customers"]:,} customers ({max_customers_segment["share_customers"]:.1%} of total)'
    )
    report_lines.append(
        f'- **Highest Risk Segment:** Segment {high_risk_segment["segment"]} has {high_risk_segment["high_recency_share"]:.1%} customers at risk of churning'
    )
    report_lines.append("")
    report_lines.append(
    ...
    for _, narrative in narratives_df.iterrows():
        report_lines.append(
            f'### Segment {narrative["segment"]}: {narrative["segment_label"]}'
        )
        report_lines.append("")
...
def main():
    ...
    customer_segments_final = data[
        ["CustomerID", "segment", "Recency", "Frequency", "Monetary"]
    ].copy()
    customer_segments_final.to_csv(
        "data/processed/customer_segments_final.csv", index=False
    )
    print(
        "✓ Customer segments final saved to: data/processed/customer_segments_final.csv"
    )
    ...
    # Save labeled data
    data_labeled = data[
        ["CustomerID", "segment", "segment_label", "Recency", "Frequency", "Monetary"]
    ].copy()
    data_labeled.to_csv("data/processed/customer_segments_labeled.csv", index=False)
    print(
        "✓ Customer segments labeled saved to: data/processed/customer_segments_labeled.csv"
    )
    ...
    markdown_report = generate_markdown_report(
        CLUSTER_SOURCE, segment_profiles, segment_narratives
    )

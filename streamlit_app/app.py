...
def create_pca_plot(rfm_scaled, labels, algorithm):
    ...
    for i, label in enumerate(unique_labels):
        if label == -1:  # Noise points for DBSCAN
            mask = labels == label
            ax.scatter(
                components_2d[mask, 0],
                components_2d[mask, 1],
                c="red",
                marker="x",
                s=100,
                alpha=0.7,
...
def create_heatmap(rfm_df, labels):
    ...
    segment_profiles = rfm_with_labels.groupby("segment")[
    ...
    for i in range(len(segment_profiles)):
        for j in range(3):
            ax.text(
                i,
                j,
                f"{segment_profiles.iloc[i, j]:.1f}",
                ha="center",
                va="center",
                color="black",
                fontweight="bold",
...
def main():
    ...
    # Header
    st.markdown(
        '<h1 class="main-header">Customer Segmentation App</h1>', unsafe_allow_html=True
    )
    ...
    algorithm = st.sidebar.selectbox(
        "Select Clustering Algorithm", ["KMeans", "DBSCAN", "Hierarchical"]
    )
    ...
    if not run_button:
        ...
        if use_sample:
            ...
            st.caption(
                f"Sample data contains {len(sample_data)} transactions from {sample_data['CustomerID'].nunique()} customers"
            )
    ...
    try:
        ...
        if not np.isnan(metrics["silhouette"]):
            ...
            st.warning(
                "⚠️ Silhouette score not available (requires at least 2 clusters)"
            )

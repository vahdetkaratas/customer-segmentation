...
def test_reporting_files_exist(project_root):
    ...
    assert segment_profiles_path.exists(), f"Segment profiles file not found: {segment_profiles_path}"
    
    customer_segments_final_path = project_root / "data" / "processed" / "customer_segments_final.csv"
    assert customer_segments_final_path.exists(), f"Customer segments final file not found: {customer_segments_final_path}"
    
    customer_segments_labeled_path = project_root / "data" / "processed" / "customer_segments_labeled.csv"
    assert customer_segments_labeled_path.exists(), f"Customer segments labeled file not found: {customer_segments_labeled_path}"
    
    # Check report files
    segment_report_md_path = project_root / "reports" / "segment_report.md"
    assert segment_report_md_path.exists(), f"Segment report markdown file not found: {segment_report_md_path}"
...
def test_segment_profiles_data_sanity(project_root):
    ...
    required_columns = [
        'segment', 'n_customers', 'share_customers', 'avg_recency', 
        'median_recency', 'avg_frequency', 'median_frequency', 
        'avg_monetary', 'median_monetary', 'revenue_share', 'high_recency_share'
    ]
    ...
    assert total_revenue_share == pytest.approx(1.0, abs=1e-6), \
    ...
    assert (profiles_df['n_customers'] > 0).all(), "Some segments have non-positive customer counts"
    assert (profiles_df['n_customers'] == profiles_df['n_customers'].astype(int)).all(), \
        "Customer counts should be integers"
    
    # Check that shares are between 0 and 1
    assert (profiles_df['share_customers'] >= 0).all() and (profiles_df['share_customers'] <= 1).all(), \
        "share_customers should be between 0 and 1"
    assert (profiles_df['revenue_share'] >= 0).all() and (profiles_df['revenue_share'] <= 1).all(), \
        "revenue_share should be between 0 and 1"
...
def test_customer_segments_final_structure(project_root):
    ...
    for col in required_columns:
        assert col in segments_df.columns, f"Customer segments final missing column: {col}"
    
    # Check that all customers have segments
    assert segments_df['segment'].notna().all(), "Some customers have missing segment labels"
    
    # Check that segment values are integers
    assert segments_df['segment'].dtype in ['int64', 'int32'], \
    ...
    for col in rfm_columns:
        assert pd.api.types.is_numeric_dtype(segments_df[col]), f"{col} should be numeric"
...
def test_customer_segments_labeled_structure(project_root):
    ...
    # Check required columns
    required_columns = ['CustomerID', 'segment', 'segment_label', 'Recency', 'Frequency', 'Monetary']
    for col in required_columns:
        assert col in labeled_df.columns, f"Customer segments labeled missing column: {col}"
    
    # Check that all customers have business labels
    assert labeled_df['segment_label'].notna().all(), "Some customers have missing business labels"
    assert (labeled_df['segment_label'] != '').all(), "Some customers have empty business labels"
    
    # Check that business labels are valid
    valid_labels = ['VIP Loyal', 'High-Value Rare Buyers', 'Regulars', 'At-Risk / Inactive', 'General']
    ...
    # Check that all customers have segments
    assert labeled_df['segment'].notna().all(), "Some customers have missing segment labels"
    
    # Check that segment values are integers
    assert labeled_df['segment'].dtype in ['int64', 'int32'], \
...
def test_segment_narratives_structure(project_root):
    ...
    if not narratives_path.exists():
        pytest.skip("Segment narratives file not found - optional GPT enrichment may not have been run")
    
    narratives_df = pd.read_csv(narratives_path)
    
    # Check required columns
    required_columns = ['segment', 'segment_label', 'short_description', 'campaign_ideas']
    for col in required_columns:
        assert col in narratives_df.columns, f"Segment narratives missing column: {col}"
    
    # Check that all segments have narratives
    assert narratives_df['segment'].notna().all(), "Some segments have missing segment numbers"
    assert narratives_df['segment_label'].notna().all(), "Some segments have missing business labels"
    assert narratives_df['short_description'].notna().all(), "Some segments have missing descriptions"
    assert narratives_df['campaign_ideas'].notna().all(), "Some segments have missing campaign ideas"
    
    # Check that descriptions and campaign ideas are not empty
    assert (narratives_df['short_description'] != '').all(), "Some segments have empty descriptions"
    assert (narratives_df['campaign_ideas'] != '').all(), "Some segments have empty campaign ideas"
    
    # Check that segment numbers are integers
    assert narratives_df['segment'].dtype in ['int64', 'int32'], \
...
def test_data_consistency_across_files(project_root):
    ...
    profiles_df = pd.read_csv(project_root / "data" / "processed" / "segment_profiles.csv")
    segments_final_df = pd.read_csv(project_root / "data" / "processed" / "customer_segments_final.csv")
    segments_labeled_df = pd.read_csv(project_root / "data" / "processed" / "customer_segments_labeled.csv")
    ...
    assert profiles_segments == final_segments == labeled_segments, \
        "Segment numbers don't match across files"
    
    # Check that customer counts match
    for segment in profiles_segments:
        profile_count = profiles_df[profiles_df['segment'] == segment]['n_customers'].iloc[0]
        final_count = len(segments_final_df[segments_final_df['segment'] == segment])
        labeled_count = len(segments_labeled_df[segments_labeled_df['segment'] == segment])
        
        assert profile_count == final_count == labeled_count, \
    ...
    assert final_customers == labeled_customers, \
...
def test_business_label_distribution(project_root):
    ...
    """
    labeled_df = pd.read_csv(project_root / "data" / "processed" / "customer_segments_labeled.csv")
    
    # Get label distribution
    label_counts = labeled_df['segment_label'].value_counts()
    
    # Check that we have at least 2 different business labels
    assert len(label_counts) >= 2, f"Only {len(label_counts)} business labels found, expected at least 2"
    
    # Check that no single label dominates (more than 80% of customers)
    max_share = label_counts.max() / len(labeled_df)
    if max_share > 0.8:
        pytest.xfail(f"Single business label dominates: {max_share:.1%} of customers have the same label")
    
    # Check that we have a reasonable number of labels (not too many, not too few)
    assert 2 <= len(label_counts) <= 10, f"Found {len(label_counts)} business labels, expected 2-10"
...
def test_segment_profiles_statistical_consistency(project_root):
    ...
    assert (profiles_df['avg_recency'] >= 0).all(), "Some average recency values are negative"
    assert (profiles_df['avg_frequency'] > 0).all(), "Some average frequency values are non-positive"
    assert (profiles_df['avg_monetary'] > 0).all(), "Some average monetary values are non-positive"
    
    # Check that medians are reasonable
    assert (profiles_df['median_recency'] >= 0).all(), "Some median recency values are negative"
    assert (profiles_df['median_frequency'] > 0).all(), "Some median frequency values are non-positive"
    assert (profiles_df['median_monetary'] > 0).all(), "Some median monetary values are non-positive"
    ...
    for segment in profiles_df['segment']:
        ...
        frequency_ratio = segment_data['avg_frequency'] / segment_data['median_frequency']
        ...
        if recency_ratio > 3 or recency_ratio < 0.33:
            pytest.xfail(f"Segment {segment} has extreme recency skew: avg/median ratio = {recency_ratio:.2f}")
        if frequency_ratio > 3 or frequency_ratio < 0.33:
            pytest.xfail(f"Segment {segment} has extreme frequency skew: avg/median ratio = {frequency_ratio:.2f}")
        if monetary_ratio > 3 or monetary_ratio < 0.33:
            pytest.xfail(f"Segment {segment} has extreme monetary skew: avg/median ratio = {monetary_ratio:.2f}")
...
def test_pathological_dataset_handling(project_root):
    ...
    if len(profiles_df) == 1:
        pytest.xfail("All customers are in one segment - this is a pathological case that may need special handling")
    
    # Check if we have extremely unbalanced segments
    max_share = profiles_df['share_customers'].max()
    if max_share > 0.9:
        pytest.xfail(f"Extremely unbalanced segments: {max_share:.1%} of customers in one segment")
    
    # Check if we have segments with very few customers
    min_customers = profiles_df['n_customers'].min()
    if min_customers < 10:
        pytest.xfail(f"Some segments have very few customers: minimum {min_customers} customers")

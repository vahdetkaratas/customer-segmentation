...
from src.app_core import (
    ...
    run_dbscan, run_hierarchical, evaluate_clusters, pca_2d
)
...
def test_load_or_sample_data_valid_csv():
    ...
    try:
        ...
        assert list(df['CustomerID']) == ['CUST_001', 'CUST_002'], "CustomerIDs should match"
...
def test_scale_rfm():
    ...
    assert list(rfm_scaled['CustomerID']) == list(rfm_df['CustomerID']), "CustomerID should be preserved"
...
def test_run_dbscan():
    ...
    assert len(labels) == len(rfm_scaled), "Should have same number of labels as data points"
    
    # Check that labels are integers
    assert all(isinstance(label, (int, np.integer)) for label in labels), "All labels should be integers"
...
def test_run_hierarchical():
    ...
    unique_labels = set(labels)
    assert len(unique_labels) == n_clusters, f"Should have exactly {n_clusters} unique labels"
...
def test_evaluate_clusters():
    ...
    cluster1 = np.random.multivariate_normal([0, 0, 0], [[1, 0, 0], [0, 1, 0], [0, 0, 1]], n_samples//3)
    cluster2 = np.random.multivariate_normal([3, 3, 3], [[1, 0, 0], [0, 1, 0], [0, 0, 1]], n_samples//3)
    cluster3 = np.random.multivariate_normal([-3, -3, -3], [[1, 0, 0], [0, 1, 0], [0, 0, 1]], n_samples//3)
    ...
    # Check required keys
    required_keys = ['silhouette', 'calinski_harabasz', 'davies_bouldin', 'n_clusters', 'n_noise']
...
def test_evaluate_clusters_single_cluster():
    ...
    assert np.isnan(metrics['silhouette']), "Silhouette should be NaN for single cluster"
    assert np.isnan(metrics['calinski_harabasz']), "Calinski-Harabasz should be NaN for single cluster"
    assert np.isnan(metrics['davies_bouldin']), "Davies-Bouldin should be NaN for single cluster"
...
def test_compute_rfm_data_cleaning():
    ...
    data = {
        ...
        'InvoiceDate': ['2023-01-01', '2023-01-02', '2023-01-03', '2023-01-04', '2023-01-05'],
    ...
    assert all(cust in rfm['CustomerID'].values for cust in expected_customers), "Should have expected customers"
...
def test_scale_rfm_preserves_customerid():
    ...
    assert list(rfm_scaled['CustomerID']) == list(rfm_df['CustomerID']), "CustomerID should be preserved exactly"
    assert rfm_scaled['CustomerID'].dtype == rfm_df['CustomerID'].dtype, "CustomerID dtype should be preserved"

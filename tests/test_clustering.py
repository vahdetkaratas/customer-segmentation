...
def test_labels_file_exists_and_count(project_root):
    ...
    assert (
        labels_df["segment"].notna().all()
    ), "Some customers have missing segment labels"

    # Extract k from filename for other tests
    # k_from_filename = int(labels_path.stem.split("_k")[1])  # Unused variable
...
def test_silhouette_score_validation(project_root):
    ...
    if silhouette <= 0.2:
        pytest.xfail(
            f"Weak cluster separation (silhouette={silhouette:.3f}) — dataset may not be clusterable"
        )

    # If we get here, silhouette is good
    assert (
        silhouette > 0.2
    ), f"Silhouette score {silhouette:.3f} is too low for good clustering"
...
def test_model_file_exists(project_root):
    ...
    try:
        ...
        assert hasattr(
            model, "n_clusters"
        ), "Loaded model does not have n_clusters attribute"
        assert hasattr(
            model, "cluster_centers_"
        ), "Loaded model does not have cluster_centers_ attribute"
...
def test_rfm_with_segments_file(project_root):
    ...
    assert rfm_merged["segment"].dtype in [
        "int64",
        "int32",
...
def test_clustering_consistency(project_root):
    ...
    assert (
        labels_customers == merged_customers
    ...
    assert (
        labels_df_sorted["segment"] == rfm_merged_sorted["segment"]
    ).all(), "Segment assignments do not match between labels and merged files"
...
def test_segment_distribution(project_root):
    ...
    assert (
        segment_counts >= min_expected
    ).all(), f"Some segments have fewer than 5% of customers (minimum expected: {min_expected:.0f})"
